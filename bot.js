...
const { TurnContext } = require("botbuilder"); // Ensure you import TurnContext
...
class EchoBot extends ActivityHandler {
  constructor(
    ...
    this.KnowOrderStatusDialog = new KnowOrderStatusDialog(
      this.conversationState,
      this.userState
    );
    this.TalkToAgentDialog = new TalkToAgentDialog(
      this.conversationState,
      this.userState
    ); // Instantiate the new dialog
  ...
  async sendSuggestedActions(turnContext) {
    const reply = MessageFactory.suggestedActions(
      ["Check my order status", "Chat with a dietitian"],
  ...
  async dispatchToIntentAsync(context) {
    ...
    const cacheKey = `CLU:${userInput}`;
    const cachedCLU = cache.get(cacheKey);

    if (cachedCLU) {
      console.log("Using cached CLU result");
      var cluResult = cachedCLU.result;
    } else {
      console.log("Calling CLU API");
      cluResult = await getCluPrediction(userInput);
      cache.set(cacheKey, { result: cluResult });
    }
    ...
    switch (topIntent) {
      ...
      case "FAQ":
        ...
        try {
          ...
          if (qnaAnswers.length > 0 && qnaAnswers[0].confidenceScore > 0.3) {
            // Adjust threshold as needed
            await context.sendActivity(qnaAnswers[0].answer);
          } else {
            await context.sendActivity(
              "I'm sorry, I couldn't find an answer to your question. Could you please rephrase?"
            );
          }
        } catch (error) {
          await context.sendActivity(
            "I'm experiencing some technical difficulties while trying to answer your question. Please try again later."
          );
        }
        if (userQuestion == "Start my weight loss program") {
          const weightLossMsg =
            "Ready to start? Click [here](https://www.personalizedhealthrx.com/questionnaires) to begin!";
      ...
      //     // get the agents data for respective tenant 2
      //     // if no agents are available and unavialble
      //     //
      //     const io = require("socket.io-client");
      //     // Use your socket server URL from your configuration
      //     const socketServerUrl =
      //       "https://stage-phrx-customerchat-nodesocketserver.azurewebsites.net/";
      //     const agentSocket = io(socketServerUrl, {
      //       reconnection: true,
      //       reconnectionAttempts: Infinity,
      //       reconnectionDelay: 5000,
      //     });
      //    // Capture and clone the conversation reference.
      //    const conversationReference = JSON.parse(
      //     JSON.stringify(TurnContext.getConversationReference(context.activity))
      //   );

      //     // Emit the event to check agent update
      //     agentSocket.emit("check_agent_update", 23);

      //     // Listen for the response event
      //     agentSocket.on("agent_status", async (data) => {
      //       // Assuming data has a key "agentStatus" with value like "away" or "unavailable"
      //       const agentStatus = data;
      //       console.log("Received agent update:", agentStatus);

      //       // Optionally disconnect after receiving the update
      //       agentSocket.disconnect();
      //       this.adapter.continueConversationAsync(conversationReference, async (newContext) => {
      //         await this.TalkToAgentDialog.run(newContext, this.dialogState, { agentStatus });
      //       });
      //     });
      ...
      case "AgentRequest":
        ...
        const io = require("socket.io-client");
        const socketServerUrl =
          "https://phrx-customerchat-nodesocketserver.azurewebsites.net/";
        const agentSocket = io(socketServerUrl, {
          reconnection: true,
          reconnectionAttempts: Infinity,
          reconnectionDelay: 5000,
        ...
        // Create a promise that will resolve when the socket emits "agent_status"
        const agentStatusPromise = new Promise((resolve, reject) => {
          // Emit the event to check for agent update
          agentSocket.emit("check_agent_update", { tenatId: 2 });

          // Listen for the response event from the socket
          agentSocket.on("agent_status", (data) => {
            resolve(data);
            agentSocket.disconnect();
          });

          // Optionally, set a timeout if you don't want to wait indefinitely
          setTimeout(() => {
        ...
        try {
          // Wait for the socket to return the agent status
          const agentStatus = await agentStatusPromise;
          console.log("Received agent update:", agentStatus);

          // Now continue in the same turn by starting the dialog with the received data
          await this.TalkToAgentDialog.run(context, this.dialogState, {
            agentStatus,
          });
        } catch (error) {
          console.error("Error waiting for socket response:", error);
          await context.sendActivity(
            "Error connecting to agent status. Please try again later."
          );
        }
        break;
      ...
      case "ChitChat":
        await context.sendActivity(
          "Hey there welcome to PersonalizedHealthRx! i am a virtual assistant, how can i help you today?"
        );
        await this.sendSuggestedActions(context);
        break;
      case "Thanks":
      ...
      case "Farewell":
        await context.sendActivity(
          "Hope i helped You today,Goodbye! Have a great day!"
        );
      ...
      case "None":
        await context.sendActivity(
          "I'm sorry, I didn't understand that. Let me help you. Choose one of the options below."
        );
        await this.sendSuggestedActions(context);
        break;
      default:
  ...
  async sendAgentInfo(context) {
    ...
    const randomText =
      agentTexts[Math.floor(Math.random() * agentTexts.length)];
    await context.sendActivity(randomText);
    await this.sendSuggestedActions(context);
  }
}
